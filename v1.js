...
const mail1 = ('sitelwil06@gmail.com');

const password1 = ('Glipa382327$');

const name1 = ('sylvain');

const lastName1 = ('lunnise');
...
(async () => {
    ...
    openManualLoginTask('Task 1', async (driver) => {
        ...
        for (const option6 of options6) {
            ...
            if (text.trim() === 'Sin permiso del INM') {
        ...
        console.log('sin permiso clicked');
        ...
        const dropdownToggleb = await driver.findElement(By.name('formalitites_subtype_id'));
        ...
        const options6b = await driver.findElements(By.css('[role="option"], .vs__dropdown-option, .vs__dropdown-menu div'));



        // Loop through and click the matching text

        for (const option6b of options6b) {

            const text = await option6b.getText();

            if (text.trim() === 'Visitante sin permiso para realizar actividades remuneradas') {

                await option6b.click();
